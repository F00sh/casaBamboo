<template>
  <div class="min-h-screen">
    <header class="sticky top-0 z-40 border-b backdrop-blur transition-transform duration-300" :class="[hiddenHeader ? '-translate-y-full' : 'translate-y-0', 'bg-[var(--card-bg)]/80','border-[var(--border-color)]']">
      <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        <a href="#" class="flex items-center gap-2 group">
          <span class="inline-block h-2 w-2 bg-emerald-500 group-hover:bg-emerald-600 transition"></span>
          <span class="text-lg font-semibold tracking-tight font-cinzel">Casa Bamboo</span>
        </a>
        <nav class="hidden md:flex items-center gap-6 text-sm font-medium">
          <a href="#apartments" class="hover:text-emerald-700 transition">Apartments</a>
          <a href="#amenities" class="hover:text-emerald-700 transition">Amenities</a>
          <a href="#location" class="hover:text-emerald-700 transition">Location</a>
        ...
        <button class="md:hidden inline-flex items-center border border-black/10 px-3 py-2 text-sm" @click="open = !open">
          <span>Menu</span>
        </button>
      </div>
    ...
    <footer class="border-t" :class="['bg-[var(--card-bg)]/70','border-[var(--border-color)]']">
      <div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-6 text-sm text-neutral-600 sm:flex-row sm:px-6 lg:px-8">
        <p>© {{ year }} Casa Bamboo — Istria, Croatia</p>
        <div class="flex items-center gap-4">
          <a href="#contact" class="hover:text-emerald-700">Contact</a>
          <a href="https://www.casa-bamboo.com/" target="_blank" rel="noopener" class="hover:text-emerald-700">Official Site</a>
        </div>
      </div>
    </footer>
  </div>
